...
async function registrarVerificacion(data) {
  ...
  // La cantidad que llega es lo que se instaló EN ESTE REGISTRO (se SUMA a lo ya instalado).
  const delta = Number(data.cantidadInstalada);
  ...
  const yaInstalado = Number(detalle.cantidad_instalada) || 0;
  const totalInstalado = yaInstalado + delta;
  if (totalInstalado > entregado) {
    throw new Error(
      'La cantidad instalada total (' + totalInstalado + ') superaría lo entregado (' + entregado +
      '). Ya había ' + yaInstalado + ' instalada(s); solo puedes agregar hasta ' + (entregado - yaInstalado) + '.'
    );
  }
  const pendiente = entregado - totalInstalado;
  ...
  // En la bitácora de verificaciones se guarda lo instalado EN ESTE registro (el delta).
  ...
  // En el detalle se guarda el TOTAL acumulado.
  const { error: eUpd } = await supabase
    .from('detalle_entregas')
    .update({ cantidad_instalada: totalInstalado, cantidad_pendiente: pendiente })
    .eq('id_detalle', data.idDetalle);
  ...
  return { idVerificacion, agregado: delta, totalInstalado, pendiente, estado: estadoLinea, timestamp };
}
