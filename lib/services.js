...
async function guardarPersona(persona, nombreQuienRegistra) {
  ...
  if (!persona.ZONA_ID) throw new Error('Debes asignar una zona a la persona.');
  if (persona.ID) {
...
async function iniciarSesionPersona(personaId, codigo) {
  ...
  let zonaNombre = '';
  if (persona.zona_id) {
    const { data: z } = await supabase.from('zonas').select('nombre').eq('id', persona.zona_id).limit(1);
    zonaNombre = z && z[0] ? z[0].nombre : '';
  }
  await registrarLog(persona.nombre, 'LOGIN_CAMPO', { personaId: persona.id });
